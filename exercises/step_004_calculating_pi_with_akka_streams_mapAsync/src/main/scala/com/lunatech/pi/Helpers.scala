...
object Helpers:
  ...
  extension [A, M](flow: Source[A, M])
    def logAtInfo[E](name: String, extract: A => E): Source[A, M] =
      flow
        .log(name, extract)
        .withAttributes(
    ...
    def logAtInfo(name: String): Source[A, M] =
      logAtInfo(name, identity)
